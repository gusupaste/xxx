...
<style scoped>
    .parentInfoStudent th ,.parentInfoStudent td {
...
export default {
    ...
    mounted:function(){
      if(localStorage.getItem('studentTabName') === 'second'){
        this.getAllPotions();
        this.getCountryPotions();
        this.getParentList();
      }
    ...
    methods:{
        ...
        deleteParentInfo:function(){
          this.ruleFormShow = false;
          this.$axios.delete(this.add_par_info + this.deleteId +'/').then(res=>{
            if(res.status == 200){
              this.$message({
                type:'success',
                message:'删除成功！'
